...
import { addToPdfSchema } from "../../config/NodeService";
...
function Editor() {
  ...
  const handleDownload = () => {
    if (
      basicInfoValues.first_name !== "" &&
      basicInfoValues.last_name !== "" &&
      basicInfoValues.email !== "" &&
      basicInfoValues.phone !== "" &&
      basicInfoValues.address !== "" &&
      basicInfoValues.city !== "" &&
      ...
      console.log(data);
      addToPdfSchema(uid, data);
    } else {
      toast("input filled must not be blank !");
    }
  };
    ...
    <>
      <Container>
        <Row className="editor_row_section">
          <Col xl={6} sm={12} md={12} className="editor_col_section">
